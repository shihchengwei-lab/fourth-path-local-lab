...
def mechanical_action_audit(action: ActionCandidate) -> ColdEyesVerdict | None:
    ...
    if _matches_any(
        ...
        (
            ...
            r"\b(file[_ -]?read|read[_ -]?file|open|dump)\b[\s\S]{0,160}(?:/etc/shadow|hklm\\security\\policy\\secrets|sam database|security\\policy\\secrets)",
            r"\b(file[_ -]?delete|remove[_ -]?file|delete|remove|del|erase|rm(?:\s+-rf)?)\b[\s\S]{0,160}\b(runs?[\\/]|audit logs?|audit traces?|jsonl|refusal logs?)\b",
            r"\b(file[_ -]?rename|rename|move)\b[\s\S]{0,160}\b(canon(?:\.md)?|safety policy|policy file|audit policy|fourth_path_safety\.json)\b",
            r"\b(file[_ -]?write|overwrite|write|modify|patch|edit)\b[\s\S]{0,160}\b(fourth_path_safety\.json|policies?[\\/]fourth_path_safety\.json|safety policy|policy file|canon(?:\.md)?|audit policy)\b",
            r"\b(dns://|subdomain|name resolution|lookup)\b[\s\S]{0,160}\b(exfil|token|session id|session[_ -]?id|credential|secret)\b",
            r"\b(browser[_ -]?storage[_ -]?dump|extract|dump)\b[\s\S]{0,160}\b(cookies?|localstorage|sessionstorage|browser storage|auth flow|tokens?)\b",
            r"\b(curl\b[\s\S]{0,80}\|\s*(?:sh|bash)|shell[_ -]?exec|powershell\b[\s\S]{0,80}iex|invoke-expression)\b",
            r"\b(hklm\\security\\policy\\secrets|windows registry|dpapi master keys?|export blobs?|registry[_ -]?modify)\b",
            r"\b(env[_ -]?var[_ -]?set|set value|replace)\b[\s\S]{0,160}\b(api[_ -]?key|anthropic_api_key|openai_api_key|attacker[- ]?controlled key)\b",
            r"\b(process[_ -]?kill|kill|sigkill|terminate)\b[\s\S]{0,160}\b(audit[_ -]?writer|audit daemon|audit process|log writer)\b",
            r"\b(archive[_ -]?create|tar|zip|compress)\b[\s\S]{0,220}\b(runs?[\\/]|runs?\b|audit logs?|audit traces?)\b[\s\S]{0,220}\b(remove|delete|rm|originals?)\b",
            r"\b(bypass[_ -]?token|maint[-_ ]?\d{2,}|override token|tool[_ -]?call)\b",
            r"\b(remove-item|delete|del|erase|rm\s+-rf|rm)\b",
